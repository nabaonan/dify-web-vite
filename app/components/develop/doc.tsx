...
const Doc = ({ appDetail }: IDocProps) => {
  ...
  const Template = useMemo(() => {
    const container = { current: null as React.ReactNode }
    const loadMDX = async () => {
      try {
        let mod: any
        const mode = appDetail?.mode
        const localeIdx = locale
        // Try to lazy load the template
        if (mode === 'chat' || mode === 'agent-chat') {
          if (localeIdx === LanguagesSupported[1])
            mod = await import('./template/template_chat.zh.mdx')
          else if (localeIdx === LanguagesSupported[7])
            mod = await import('./template/template_chat.ja.mdx')
          else
            mod = await import('./template/template_chat.en.mdx')
        }
        else if (mode === 'advanced-chat') {
          if (localeIdx === LanguagesSupported[1])
            mod = await import('./template/template_advanced_chat.zh.mdx')
          else if (localeIdx === LanguagesSupported[7])
            mod = await import('./template/template_advanced_chat.ja.mdx')
          else
            mod = await import('./template/template_advanced_chat.en.mdx')
        }
        else if (mode === 'workflow') {
          if (localeIdx === LanguagesSupported[1])
            mod = await import('./template/template_workflow.zh.mdx')
          else if (localeIdx === LanguagesSupported[7])
            mod = await import('./template/template_workflow.ja.mdx')
          else
            mod = await import('./template/template_workflow.en.mdx')
        }
        else if (mode === 'completion') {
          if (localeIdx === LanguagesSupported[1])
            mod = await import('./template/template.zh.mdx')
          else if (localeIdx === LanguagesSupported[7])
            mod = await import('./template/template.ja.mdx')
          else
            mod = await import('./template/template.en.mdx')
        }
        if (mod?.default) {
          const Component = mod.default
          container.current = <Component appDetail={appDetail} variables={variables} inputs={inputs} />
        }
      }
      catch {
        container.current = <APIDocFallback appDetail={appDetail} />
      }
    }
    loadMDX()
...
// Fallback component when MDX templates fail to load
const APIDocFallback = ({ appDetail }: { appDetail: any }) => {
  const { t } = useTranslation()
  return (
    <div className="p-4">
      <h1>{t('appApi.develop.apiDocument') || 'API Documentation'}</h1>
      <p className="text-text-tertiary mt-2">
        {t('appApi.develop.apiDescription') || 'API documentation for your application.'}
      </p>
      {appDetail?.api_base_url && (
        <div className="mt-4">
          <h3>Base URL</h3>
          <code className="block mt-1 p-2 bg-components-input-bg-normal rounded">{appDetail.api_base_url}</code>
        </div>
      )}
    </div>
  )
